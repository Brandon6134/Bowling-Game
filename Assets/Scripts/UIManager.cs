...
public class UIManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Debug.Log(velocityBarRectTransform.position.x);
    }

    // Update is called once per frame
    void Update()
    {
        if (playerControllerScript.spacePressed)
        {
    ...
    {
        //for every +2 in height, increase pos Y by 1 to keep the base position of the rectangle the same location but increase in height the other direction
        //float newYPos = Mathf.Lerp(minY,maxY,tBar);
        //rect.sizeDelta = new (100,rect.sizeDelta[1] - (newYPos-rect.position.y)*2);

        velocityBar.SetActive(true);

        rect.sizeDelta = new (100, Mathf.Lerp(minY,maxY,tBar));
        
        //rect.position = new Vector3(rect.position.x,newYPos,rect.position.z);
    ...
    public (bool,bool) AnnounceScore(int pinsHit, bool isSpare, bool isStrike)
    {
        ...
        {
            text = "Strike!";
            audioSource.PlayOneShot(announcePinsHitSFX[2]);
            isStrike=false;
        }
        else if (isSpare)
        {
            text = "Spare!";
            audioSource.PlayOneShot(announcePinsHitSFX[1]);
            isSpare=false;
        }
        else if (pinsHit==0)
        {
            text = "Miss..";
            audioSource.PlayOneShot(announcePinsHitSFX[3]);
        }
        else
        {
            text = pinsHit.ToString() + " pins";
            audioSource.PlayOneShot(announcePinsHitSFX[0]);
        }
